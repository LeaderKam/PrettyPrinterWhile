/**
 * generated by Xtext 2.19.0
 */
package pretty.generator;

import com.google.common.collect.Iterables;
import java.util.ArrayList;
import java.util.List;
import org.eclipse.emf.common.util.EList;
import org.eclipse.emf.ecore.EObject;
import org.eclipse.emf.ecore.resource.Resource;
import org.eclipse.xtend2.lib.StringConcatenation;
import org.eclipse.xtext.generator.AbstractGenerator;
import org.eclipse.xtext.generator.IFileSystemAccess2;
import org.eclipse.xtext.generator.IGeneratorContext;
import org.eclipse.xtext.xbase.lib.IteratorExtensions;
import pretty.wh.Affectation;
import pretty.wh.Command;
import pretty.wh.Commands;
import pretty.wh.Expr;
import pretty.wh.For;
import pretty.wh.Foreach;
import pretty.wh.Function;
import pretty.wh.If;
import pretty.wh.Lexpr;
import pretty.wh.Nop;
import pretty.wh.Program;
import pretty.wh.While;

/**
 * Generates code from your model files on save.
 * 
 * See https://www.eclipse.org/Xtext/documentation/303_runtime_concepts.html#code-generation
 */
@SuppressWarnings("all")
public class WhGenerator extends AbstractGenerator {
  public static final int INDENT_ALL = 0;
  
  public static final int INDENT_FOR = 1;
  
  public static final int INDENT_WHILE = 2;
  
  public static final int INDENT_IF = 3;
  
  public static final int INDENT_FOREACH = 4;
  
  public static final Integer DEFAULT_ALL = Integer.valueOf(2);
  
  public static final Integer DEFAULT_FOR = Integer.valueOf(Integer.MIN_VALUE);
  
  public static final Integer DEFAULT_WHILE = Integer.valueOf(Integer.MIN_VALUE);
  
  public static final Integer DEFAULT_IF = Integer.valueOf(Integer.MIN_VALUE);
  
  public static final Integer DEFAULT_FOREACH = Integer.valueOf(Integer.MIN_VALUE);
  
  private String indentAll = "";
  
  private String indentFor = "";
  
  private String indentWhile = "";
  
  private String indentIf = "";
  
  private String indentForeach = "";
  
  private int i;
  
  private List<Integer> indentations = new ArrayList<Integer>();
  
  @Override
  public void doGenerate(final Resource resource, final IFileSystemAccess2 fsa, final IGeneratorContext context) {
    WhGenerator.init(this.indentations);
    this.doGenerate(resource, fsa, context, "", this.indentations);
  }
  
  public void doGenerate(final Resource resource, final IFileSystemAccess2 fsa, final IGeneratorContext context, final String output, final List<Integer> indentations) {
    this.indentations = indentations;
    this.calcIndent(this.indentations);
    Iterable<Program> _filter = Iterables.<Program>filter(IteratorExtensions.<EObject>toIterable(resource.getAllContents()), Program.class);
    for (final Program e : _filter) {
      boolean _equals = output.equals("");
      if (_equals) {
        System.out.println(this.compile(e));
      } else {
        fsa.generateFile(output, this.compile(e));
      }
    }
  }
  
  public CharSequence compile(final Program p) {
    StringConcatenation _builder = new StringConcatenation();
    {
      EList<Function> _functions = p.getFunctions();
      for(final Function f : _functions) {
        CharSequence _compile = this.compile(f);
        _builder.append(_compile);
        _builder.newLineIfNotEmpty();
      }
    }
    return _builder;
  }
  
  public CharSequence compile(final Function f) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("function ");
    String _name = f.getName();
    _builder.append(_name);
    _builder.append(" :");
    _builder.newLineIfNotEmpty();
    _builder.append("read ");
    {
      EList<String> _variable = f.getDefinition().getRead().getVariable();
      boolean _hasElements = false;
      for(final String vr : _variable) {
        if (!_hasElements) {
          _hasElements = true;
        } else {
          _builder.appendImmediate(",", "");
        }
        _builder.append(vr);
      }
    }
    _builder.newLineIfNotEmpty();
    _builder.append("%");
    _builder.newLine();
    CharSequence _compile = this.compile(f.getDefinition().getCommands(), this.indentAll);
    _builder.append(_compile);
    _builder.newLineIfNotEmpty();
    _builder.append("%");
    _builder.newLine();
    _builder.append("write ");
    {
      EList<String> _variable_1 = f.getDefinition().getWrite().getVariable();
      boolean _hasElements_1 = false;
      for(final String vw : _variable_1) {
        if (!_hasElements_1) {
          _hasElements_1 = true;
        } else {
          _builder.appendImmediate(",", "");
        }
        _builder.append(vw);
      }
    }
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final Commands c, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    {
      EList<Command> _commands = c.getCommands();
      boolean _hasElements = false;
      for(final Command command : _commands) {
        if (!_hasElements) {
          _hasElements = true;
        } else {
          _builder.appendImmediate(" ;", "");
        }
        _builder.append(indent);
        CharSequence _compile = this.compile(((Command) command), indent);
        _builder.append(_compile);
        _builder.newLineIfNotEmpty();
      }
    }
    return _builder;
  }
  
  public CharSequence compile(final Command c, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    {
      EObject _command = c.getCommand();
      if ((_command instanceof Nop)) {
        _builder.append("nop");
        _builder.newLineIfNotEmpty();
      } else {
        EObject _command_1 = c.getCommand();
        if ((_command_1 instanceof If)) {
          EObject _command_2 = c.getCommand();
          CharSequence _compile = this.compile(((If) _command_2), indent);
          _builder.append(_compile);
          _builder.newLineIfNotEmpty();
        } else {
          EObject _command_3 = c.getCommand();
          if ((_command_3 instanceof Affectation)) {
            EObject _command_4 = c.getCommand();
            CharSequence _compile_1 = this.compile(((Affectation) _command_4));
            _builder.append(_compile_1);
            _builder.newLineIfNotEmpty();
          } else {
            EObject _command_5 = c.getCommand();
            if ((_command_5 instanceof While)) {
              EObject _command_6 = c.getCommand();
              CharSequence _compile_2 = this.compile(((While) _command_6), indent);
              _builder.append(_compile_2);
              _builder.newLineIfNotEmpty();
            } else {
              EObject _command_7 = c.getCommand();
              if ((_command_7 instanceof For)) {
                EObject _command_8 = c.getCommand();
                CharSequence _compile_3 = this.compile(((For) _command_8), indent);
                _builder.append(_compile_3);
                _builder.newLineIfNotEmpty();
              } else {
                EObject _command_9 = c.getCommand();
                if ((_command_9 instanceof Foreach)) {
                  EObject _command_10 = c.getCommand();
                  CharSequence _compile_4 = this.compile(((Foreach) _command_10), indent);
                  _builder.append(_compile_4);
                  _builder.newLineIfNotEmpty();
                }
              }
            }
          }
        }
      }
    }
    return _builder;
  }
  
  public CharSequence compile(final While w, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("while ");
    CharSequence _compile = this.compile(w.getExpr());
    _builder.append(_compile);
    _builder.append(" do");
    _builder.newLineIfNotEmpty();
    Object _compile_1 = this.compile(w.getCommands(), (indent + this.indentWhile));
    _builder.append(_compile_1);
    _builder.newLineIfNotEmpty();
    _builder.append(indent);
    _builder.append("od");
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final If i, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("if ");
    CharSequence _compile = this.compile(i.getExpr());
    _builder.append(_compile);
    _builder.append(" then");
    _builder.newLineIfNotEmpty();
    Object _compile_1 = this.compile(i.getCommands1(), (indent + this.indentIf));
    _builder.append(_compile_1);
    _builder.newLineIfNotEmpty();
    {
      Commands _commands2 = i.getCommands2();
      boolean _tripleNotEquals = (_commands2 != null);
      if (_tripleNotEquals) {
        _builder.append(indent);
        _builder.append("else");
        _builder.newLineIfNotEmpty();
        Object _compile_2 = this.compile(i.getCommands2(), (indent + this.indentIf));
        _builder.append(_compile_2);
        _builder.newLineIfNotEmpty();
      }
    }
    _builder.append(indent);
    _builder.append("fi");
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final Affectation a) {
    StringConcatenation _builder = new StringConcatenation();
    {
      EList<String> _affectations = a.getAffectations();
      boolean _hasElements = false;
      for(final String nom : _affectations) {
        if (!_hasElements) {
          _hasElements = true;
        } else {
          _builder.appendImmediate(",", "");
        }
        _builder.append(nom);
      }
    }
    _builder.append(" := ");
    {
      EList<Expr> _valeurs = a.getValeurs();
      boolean _hasElements_1 = false;
      for(final Expr expr : _valeurs) {
        if (!_hasElements_1) {
          _hasElements_1 = true;
        } else {
          _builder.appendImmediate(",", "");
        }
        CharSequence _compile = this.compile(expr);
        _builder.append(_compile);
      }
    }
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final For f, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("for ");
    CharSequence _compile = this.compile(f.getExpr());
    _builder.append(_compile);
    _builder.append(" do");
    _builder.newLineIfNotEmpty();
    Object _compile_1 = this.compile(f.getCommands(), (indent + this.indentFor));
    _builder.append(_compile_1);
    _builder.newLineIfNotEmpty();
    _builder.append(indent);
    _builder.append("od");
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final Foreach f, final String indent) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("foreach ");
    CharSequence _compile = this.compile(f.getExpr1());
    _builder.append(_compile);
    _builder.append(" in ");
    CharSequence _compile_1 = this.compile(f.getExpr2());
    _builder.append(_compile_1);
    _builder.append(" do");
    _builder.newLineIfNotEmpty();
    Object _compile_2 = this.compile(f.getCommands(), (indent + this.indentForeach));
    _builder.append(_compile_2);
    _builder.newLineIfNotEmpty();
    _builder.append(indent);
    _builder.append("od");
    _builder.newLineIfNotEmpty();
    return _builder;
  }
  
  public CharSequence compile(final Expr e) {
    StringConcatenation _builder = new StringConcatenation();
    {
      String _valeur = e.getValeur();
      boolean _tripleNotEquals = (_valeur != null);
      if (_tripleNotEquals) {
        String _string = e.getValeur().toString();
        _builder.append(_string);
      } else {
        if (((e.getOpe().toString().equals("and") || e.getOpe().toString().equals("or")) || e.getOpe().toString().equals("=?"))) {
          _builder.append("(");
          Object _compile = this.compile(e.getEx1());
          _builder.append(_compile);
          _builder.append(" ");
          String _string_1 = e.getOpe().toString();
          _builder.append(_string_1);
          _builder.append(" ");
          Object _compile_1 = this.compile(e.getEx2());
          _builder.append(_compile_1);
          _builder.append(")");
        } else {
          if (((e.getOpe().toString().equals("tl") || e.getOpe().toString().equals("!")) || e.getOpe().toString().equals("hd"))) {
            _builder.append("(");
            String _string_2 = e.getOpe().toString();
            _builder.append(_string_2);
            _builder.append(" ");
            Expr _expr = e.getExpr();
            Object _compile_2 = this.compile(((Expr) _expr));
            _builder.append(_compile_2);
            _builder.append(")");
          } else {
            _builder.append("(");
            String _string_3 = e.getOpe().toString();
            _builder.append(_string_3);
            _builder.append(" ");
            Lexpr _lexpr = e.getLexpr();
            CharSequence _compile_3 = this.compile(((Lexpr) _lexpr));
            _builder.append(_compile_3);
            _builder.append(")");
          }
        }
      }
    }
    return _builder;
  }
  
  public CharSequence compile(final Lexpr l) {
    StringConcatenation _builder = new StringConcatenation();
    {
      EList<Expr> _exprs = l.getExprs();
      boolean _hasElements = false;
      for(final Expr expr : _exprs) {
        if (!_hasElements) {
          _hasElements = true;
        } else {
          _builder.appendImmediate(" ", "");
        }
        Object _compile = this.compile(expr);
        _builder.append(_compile);
      }
    }
    return _builder;
  }
  
  public static void init(final List<Integer> integers) {
    integers.add(WhGenerator.INDENT_ALL, WhGenerator.DEFAULT_ALL);
    integers.add(WhGenerator.INDENT_FOR, WhGenerator.DEFAULT_FOR);
    integers.add(WhGenerator.INDENT_WHILE, WhGenerator.DEFAULT_WHILE);
    integers.add(WhGenerator.INDENT_IF, WhGenerator.DEFAULT_IF);
    integers.add(WhGenerator.INDENT_FOREACH, WhGenerator.DEFAULT_FOREACH);
  }
  
  public void calcIndent(final List<Integer> integers) {
    for (this.i = 0; (this.i < (this.indentations.get(WhGenerator.INDENT_ALL)).intValue()); this.i++) {
      {
        String _indentAll = this.indentAll;
        this.indentAll = (_indentAll + " ");
        String _indentFor = this.indentFor;
        this.indentFor = (_indentFor + " ");
        String _indentWhile = this.indentWhile;
        this.indentWhile = (_indentWhile + " ");
        String _indentIf = this.indentIf;
        this.indentIf = (_indentIf + " ");
        String _indentForeach = this.indentForeach;
        this.indentForeach = (_indentForeach + " ");
      }
    }
    Integer _get = this.indentations.get(WhGenerator.INDENT_FOR);
    boolean _notEquals = ((_get).intValue() != Integer.MIN_VALUE);
    if (_notEquals) {
      this.indentFor = "";
    }
    for (this.i = 0; (this.i < (this.indentations.get(WhGenerator.INDENT_FOR)).intValue()); this.i++) {
      String _indentFor = this.indentFor;
      this.indentFor = (_indentFor + " ");
    }
    Integer _get_1 = this.indentations.get(WhGenerator.INDENT_WHILE);
    boolean _notEquals_1 = ((_get_1).intValue() != Integer.MIN_VALUE);
    if (_notEquals_1) {
      this.indentWhile = "";
    }
    for (this.i = 0; (this.i < (this.indentations.get(WhGenerator.INDENT_WHILE)).intValue()); this.i++) {
      String _indentWhile = this.indentWhile;
      this.indentWhile = (_indentWhile + " ");
    }
    Integer _get_2 = this.indentations.get(WhGenerator.INDENT_IF);
    boolean _notEquals_2 = ((_get_2).intValue() != Integer.MIN_VALUE);
    if (_notEquals_2) {
      this.indentIf = "";
    }
    for (this.i = 0; (this.i < (this.indentations.get(WhGenerator.INDENT_IF)).intValue()); this.i++) {
      String _indentIf = this.indentIf;
      this.indentIf = (_indentIf + " ");
    }
    Integer _get_3 = this.indentations.get(WhGenerator.INDENT_FOREACH);
    boolean _notEquals_3 = ((_get_3).intValue() != Integer.MIN_VALUE);
    if (_notEquals_3) {
      this.indentForeach = "";
    }
    for (this.i = 0; (this.i < (this.indentations.get(WhGenerator.INDENT_FOREACH)).intValue()); this.i++) {
      String _indentForeach = this.indentForeach;
      this.indentForeach = (_indentForeach + " ");
    }
  }
}
